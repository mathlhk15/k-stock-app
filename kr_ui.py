...
6. 마지막에 "(본 요약은 AI 생성 참고용이며 투자 권유가 아닙니다)" 포함"""
        ...
        def _call_claude(prompt):
            try:
                ...
                r = _req.post(
                    ...
                    data=_body, timeout=30,
                )
                d = r.json()
                if "error" in d:
                    return "오류: " + str(d["error"].get("message", d["error"]))
        ...
        def _call_gpt(prompt):
            try:
                ...
                r = _req.post(
                    ...
                    headers={
                        "Content-Type": "application/json; charset=utf-8",
                        "Authorization": "Bearer " + st.secrets.get("OPENAI_API_KEY", ""),
                    },
                    data=_body, timeout=30,
                )
                d = r.json()
        ...
        def _call_gemini(prompt):
            try:
                ...
                r = _req.post(
                    "https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=" + _key,
